...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !SUPABASE_SERVICE_ROLE_KEY) {
  throw new Error("Faltan variables de entorno de Supabase para la QA buyer library.");
}
...
const PORT = Number(process.env.BUYER_LIBRARY_QA_PORT || 3217);
const BASE_URL = `http://127.0.0.1:${PORT}`;

const QA_PASSWORD = "ForjaDevQA!2026";
const BUYER_EMAIL = "qa-buyer-library@forjadev.local";
const EMPTY_BUYER_EMAIL = "qa-buyer-library-empty@forjadev.local";
const SELLER_EMAIL = "qa-buyer-library-seller@forjadev.local";
...
async function waitForServer(timeoutMs = 30000) {
  ...
  throw new Error("No se pudo iniciar el servidor local para la QA buyer library.");
}
...
async function ensureVendor(userId) {
  ...
  const { data: vendor, error } = await adminSupabase
    ...
    .insert([
      {
        user_id: userId,
        store_name: "QA Buyer Library Store",
        slug: `qa-buyer-library-${Date.now()}`,
        bio: "QA Buyer Library Store",
      },
    ])
    .select("id")
    .single();
...
async function createApprovedProduct(vendorId, titlePrefix, slugPrefix) {
    ...
    .insert([
      {
        ...
        short_description: "Producto QA buyer library.",
        description: "Producto QA buyer library.",
        price_cents: 2400,
        is_free: false,
    ...
    .insert([
      {
        ...
        version: "1.0.0",
        changelog: "Release activa QA buyer library.",
  ...
  const filePath = await uploadStorageZip({
    ...
    fileName: "qa-buyer-library.zip",
    contents: Buffer.from("PK-QA-BUYER-LIBRARY"),
  });
  ...
  const { error: fileError } = await adminSupabase.from("product_files").insert([
    {
      ...
      file_name: "qa-buyer-library.zip",
      file_size_bytes: Buffer.byteLength("PK-QA-BUYER-LIBRARY"),
    },
  ]);
    ...
    .insert([
      {
        ...
        product_id: productId,
        user_id: buyerUserId,
        license_key: `QA-LIB-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
...
async function main() {
  ...
  try {
    ...
    const buyerUser = await ensureUser({
      ...
      role: "buyer",
      displayName: "QA Buyer Library",
      username: "qa_buyer_library",
    });
    const emptyBuyerUser = await ensureUser({
      email: EMPTY_BUYER_EMAIL,
      password: QA_PASSWORD,
      role: "buyer",
      displayName: "QA Buyer Library Empty",
      username: "qa_buyer_library_empty",
    });
    const sellerUser = await ensureUser({
      email: SELLER_EMAIL,
      password: QA_PASSWORD,
      role: "seller",
      displayName: "QA Buyer Library Seller",
      username: "qa_buyer_library_seller",
    });

    const vendor = await ensureVendor(sellerUser.id);
    const activeProduct = await createApprovedProduct(vendor.id, "QA Library Active", "qa-library-active");
    const revokedProduct = await createApprovedProduct(vendor.id, "QA Library Revoked", "qa-library-revoked");
    const noPurchaseProduct = await createApprovedProduct(vendor.id, "QA Library Locked", "qa-library-locked");
    ...
    const anonymousLicensesResponse = await request("/licenses");
    recordCase(
      results,
      "anonimo_bloqueado_en_licenses",
      "critical",
      anonymousLicensesResponse.status >= 300 &&
        anonymousLicensesResponse.status < 400 &&
        (anonymousLicensesResponse.headers.get("location") || "").includes("/login"),
      `status=${anonymousLicensesResponse.status}, location=${anonymousLicensesResponse.headers.get("location")}`,
    ...
    const licensesHtml = await licensesResponse.text();

    recordCase(
      results,
      "buyer_ve_biblioteca_con_ownership_real",
      "critical",
      licensesResponse.status === 200 &&
        licensesHtml.includes(activeProduct.title) &&
        licensesHtml.includes(revokedProduct.title) &&
        !licensesHtml.includes(noPurchaseProduct.title),
      `status=${licensesResponse.status}`,
      "La biblioteca debe mostrar solo productos poseidos"
    );

    recordCase(
      results,
      "buyer_ve_estado_de_acceso_y_licencia",
      "critical",
      licensesHtml.includes("Acceso disponible") &&
        (licensesHtml.includes("Licencia activa") ||
          licensesHtml.includes("Licencia <!-- -->activa")) &&
        licensesHtml.includes("Licencia revocada") &&
        licensesHtml.includes("Tu licencia para este producto esta revocada"),
      "HTML incluye ownership, acceso y bloqueo revocado",
      "La biblioteca debe explicar acceso y licencia por producto"
    );

    recordCase(
      results,
      "buyer_ve_redescarga_y_continuidad",
      "critical",
      licensesHtml.includes("Descargar de nuevo") &&
        licensesHtml.includes(`/orders?highlightOrder=${activePurchase.order.id}`) &&
        licensesHtml.includes(`/support?product=${activeProduct.id}`),
      "HTML incluye redescarga y enlaces contextuales",
      "La biblioteca debe conectar con pedidos y soporte"
    );

    const emptyLibraryResponse = await request("/licenses", {
      headers: { cookie: emptyBuyerCookie },
    });
    const emptyLibraryHtml = await emptyLibraryResponse.text();
    recordCase(
      results,
      "buyer_sin_compra_ve_empty_state_claro",
      "high",
      emptyLibraryResponse.status === 200 &&
        emptyLibraryHtml.includes("Tu biblioteca aun esta vacia."),
      `status=${emptyLibraryResponse.status}`,
      "Debe existir empty state claro para buyers sin ownership"
    );
    ...
    recordCase(
      results,
      "redescarga_valida_desde_biblioteca",
      "critical",
    ...
    recordCase(
      results,
      "descarga_bloqueada_por_licencia_revocada_desde_biblioteca",
      "critical",
      ...
      "La API debe bloquear accesos con licencia revocada"
    );
    ...
    recordCase(
      results,
      "descarga_bloqueada_sin_compra_desde_biblioteca",
      "critical",
      ...
      "La API debe bloquear productos no poseidos"
    );
